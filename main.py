...
class LogSearcher:
	...
	def search_in_file(self, file_path: Path, search_pattern: Pattern, extract_archives: bool, max_depth: int = 5, current_depth: int = 0) -> bool:
		"""
	...
	def is_archive(self, file_path: Path) -> bool:
		...
		archive_extensions = ['.zip', '.tar', '.gz', '.bz2', '.tgz', '.tbz2']
	...
	def search_in_archive(self, archive_path: Path, search_pattern: Pattern, extract_archives: bool, max_depth: int, current_depth: int) -> bool:
		"""Поиск в архиве"""
		found = False
		try:
			if archive_path.suffix.lower() in ['.zip']:
			...
			elif archive_path.suffix.lower() in ['.tar', '.tgz', '.tbz2']:
				...
				# Обрабатываем как обычный сжатый файл
				found = self.search_in_text_file(archive_path, search_pattern)
		except Exception as e:
			print(f"Ошибка при обработке архива {archive_path}: {e}")
		return found

	def search_in_zip(self, zip_path: Path, search_pattern: Pattern, extract_archives: bool, max_depth: int, current_depth: int) -> bool:
			...
			with zipfile.ZipFile(zip_path, 'r') as zip_ref:
				for file_info in zip_ref.infolist():
					if not file_info.is_dir():
						...
						if extract_archives and file_extension in ['.zip', '.tar', '.gz', '.bz2', '.tgz', '.tbz2']:
	...
	def search_in_tar(self, tar_path: Path, search_pattern: Pattern, extract_archives: bool, max_depth: int, current_depth: int) -> bool:
	...
	def search_directory(self, directory: Path, search_pattern: Pattern, file_patterns: List[str], recursive: bool, extract_archives: bool, max_depth: int = 10):
...
def main():
	...
	parser.add_argument('-f', '--file-patterns', type=str, default='*.log,*.txt',
	...
	if search_path.is_file():
		...
		# Поиск в директории
		searcher.search_directory(search_path, search_pattern, file_patterns, args.recursive, args.extract_archives, args.max_depth)
