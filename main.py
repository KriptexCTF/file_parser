...
class LogSearcher:
    ...
    def is_archive(self, file_path: Path) -> bool:
        ...
        return file_path.suffix.lower() in archive_extensions or \
               (file_path.suffixes and '.'.join(file_path.suffixes[-2:]).lower() in ['.tar.gz', '.tar.bz2'])
    
    def get_tar_mode(self, file_path: Path) -> str:
        """Определяет режим открытия TAR архива"""
        suffixes = file_path.suffixes
        if not suffixes:
            return 'r'
        
        # Проверяем комбинированные расширения
        if len(suffixes) >= 2:
            last_two = '.'.join(suffixes[-2:]).lower()
            if last_two == '.tar.gz' or last_two == '.tgz':
                return 'r:gz'
            elif last_two == '.tar.bz2' or last_two == '.tbz2':
                return 'r:bz2'
        
        # Проверяем одиночные расширения
        last_suffix = suffixes[-1].lower()
        if last_suffix == '.gz' or last_suffix == '.tgz':
            return 'r:gz'
        elif last_suffix == '.bz2' or last_suffix == '.tbz2':
            return 'r:bz2'
        elif last_suffix == '.tar':
            return 'r'
    ...
    def search_in_tar(self, tar_path: Path, search_pattern: Pattern, 
        ...
        try:
            ...
            print(f"Ошибка чтения TAR архива {tar_path}: {e}")
        except Exception as e:
